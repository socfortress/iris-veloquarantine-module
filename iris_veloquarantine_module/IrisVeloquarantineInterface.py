...
import iris_veloquarantine_module.IrisVeloquarantineConfig as interface_conf
from iris_veloquarantine_module.veloquarantine_handler.veloquarantine_handler import VeloquarantineHandler


class IrisVeloquarantineInterface(IrisModuleInterface):
    """
    Provide the interface between Iris and veloquarantineHandler
    """
    name = "IrisVeloquarantineInterface"
    ...
    def register_hooks(self, module_id: int):
        ...
        if module_conf.get('veloquarantine_on_create_hook_enabled'):
        ...
        if module_conf.get('veloquarantine_on_update_hook_enabled'):
        ...
        if module_conf.get('veloquarantine_manual_hook_enabled'):
            status = self.register_to_hook(module_id, iris_hook_name='on_manual_trigger_ioc',
                                           manual_hook_name='Get veloquarantine insight')
            if status.is_failure():
                self.log.error(status.get_message())
                self.log.error(status.get_data())
    ...
    def _handle_ioc(self, data) -> InterfaceStatus.IIStatus:
        ...
        veloquarantine_handler = VeloquarantineHandler(mod_config=self.module_dict_conf,
        ...
        for element in data:
            ...
            if 'domain' in element.ioc_type.type_name:
                status = veloquarantine_handler.handle_domain(ioc=element)
            ...
            # elif element.ioc_type.type_name in etc...
                ...
                self.log.error(f'IOC type {element.ioc_type.type_name} not handled by veloquarantine module. Skipping')
